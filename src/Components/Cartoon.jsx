...
const Cartoon = ({ cartoon }) => {
  const { image, name, language, id } = cartoon;

  return (
    <div>
      <Link to={`/carttondetails/${id}`}>
        <div className="mt-4  border-4 relative max-h- hover:scale-105  transition-transform max-h-[500px] rounded-md shadow-md dark:bg-gray-100  dark:text-gray-800">
          ...
          <div className="flex flex-col justify-between p-6 space-y-8">
            ...
            <button
              type="button"
              className="flex hover:bg-green-600 items-center justify-center w-full p-3 font-semibold tracking-wide rounded-md dark:bg-violet-600 dark:text-gray-50"
            >
              <a href="">Watch Now</a>
            </button>
          </div>
